...
파티 보너스는 다음과 같이 적용된다.\
파티 내에 있는 역할군 종류 하나마다, 힘(STR), 활력(VIT), 민첩(DEX), 정신(MND), 지능(INT)에 대해 각각 1% 증가한다.\
(힘, 활력, 민첩, 정신, 지능은 주 스탯에 해당한다.)\
최대 5%까지 증가한다.
...
음식에는 레벨 조율이 적용되지 않으므로: 버전에 따라서만 결정된다.\
